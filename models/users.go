...
type Users struct {
	...
	FullName string `orm:"column(full_name);size(32);null" validate:"required,max=100"`
	Email    string `orm:"column(email);size(32);null" validate:"required,email,max=50"`
	Telp     string `orm:"column(telp);size(15);null" validate:"number,max=15"`
	Password string `orm:"column(password);size(64)" validate:"required,max=100"`
	RolesId  *Roles `orm:"column(roles_id);null;rel(fk)" validate:"number" `
}
...
// GetUsersByEmail  retrieves Users by Email. Returns error if
// Id doesn't exist
func GetUsersByEmail(e string) (v *Users, err error) {
	var user Users
	o := orm.NewOrm()
	qs := o.QueryTable("users")
	if err = qs.Filter("email", e).RelatedSel().One(&user); err == nil {
		return &user, nil
	}
	return nil, err
}
